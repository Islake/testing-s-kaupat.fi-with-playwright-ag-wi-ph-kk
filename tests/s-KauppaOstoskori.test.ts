...
test('Add item to cart', async ({ page }) => {
    ...
    // Click the "Siirry eteenpäin" button to move to Ostoskori
    await page.click('a[data-test-id="continue-order-button"]');

    // Click the "+" button to add another of the same item
    await page.waitForSelector('button[data-test-id="quantity-modifier__add"]')
    await page.click('button[data-test-id="quantity-modifier__add"]');

    // Click the comment button to uncheck that the item can be replaced with similar product
    await page.click('button[data-test-id="add-comment-btn"]');

    // Write a comment to the text field
    await page.fill('textarea[data-test-id="text-area-input"]', 'I wish the item is packed well so it does not break!');

    // Save the comment
    await page.click('button[data-test-id="save-btn"]');
});
